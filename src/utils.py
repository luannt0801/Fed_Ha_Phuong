...
def get_datasets(datasetname, **kwargs):
    ...
    if datasetname == "FashionMnist":
        ...
        #                                   transforms.ToTensor(),
        #                                 ])
        trainset = torchvision.datasets.ImageFolder(root='D:\\Phuong ham hamm\\FedNH\\data\\tumor\\Training', transform= train_brain_trans)
        testset = torchvision.datasets.ImageFolder(root='D:\\Phuong ham hamm\\FedNH\\data\\tumor\Testing', transform= test_brain_trans)
        trainset.targets = torch.tensor(trainset.targets)
        testset.targets = torch.tensor(testset.targets)
    else:
